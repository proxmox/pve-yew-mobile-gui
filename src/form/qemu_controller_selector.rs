...
fn parse_device_name(s: &str) -> Result<(&'static str, u32), Error> {
...
impl ManagedField for QemuControllerSelectorField {
    ...
    fn value_changed(&mut self, ctx: &ManagedFieldContext<Self>) {
        let state = ctx.state();
        match &state.value {
            Value::String(s) => match parse_device_name(s) {
                Ok((controller, id)) => {
    ...
    fn update(&mut self, _ctx: &ManagedFieldContext<Self>, msg: Self::Message) -> bool {
        match msg {
